...
import { posFormStateAtom } from "../../states/posFormAtom"
...
const StepperItem = React.memo(({ step, currentStep, isClickable }: StepperItemProps) => (
  <Stepper.Item 
    step={step.id} 
  ...
  >
    ...
    >
      <Stepper.Indicator
        className="flex h-9 w-9 items-center justify-center rounded-full text-sm font-medium shadow-sm border border-gray-200 data-[state=completed]:border-blue-500 data-[state=completed]:shadow-blue-100"
      >
        {currentStep > step.id ? <IconCheck className="h-4 w-4" /> : step.id}
      </Stepper.Indicator>
      <span className={`text-base font-medium ${currentStep > step.id ? "text-blue-600" : ""}`}>
        {step.title}
      </span>
    </Stepper.Trigger>
  ...
  <div className="flex flex-col p-4 border-t sticky bottom-0 bg-white">
    ...
    <div className="flex justify-between">
      ...
      <Button 
        ...
        // Fixed: Don't disable on last step, allow save functionality
...
const PosCreateStepper: React.FC<PosCreateStepperProps> = ({ children }) => {
  ...
  React.useEffect(() => {
    if (selectedStep && !stepsWithIds.some(step => step.value === selectedStep)) {
  ...
  return (
    <div className="flex h-full">
      ...
      />
      <div className={`flex-1 overflow-auto p-6 ${LAYOUT.CONTENT_MAX_HEIGHT}`}>{children}</div>
    </div>
  )
}

interface PosDetailLayoutProps {
  children: React.ReactNode
...
export const PosDetailLayout: React.FC<PosDetailLayoutProps> = ({ 
  children, 
  actions, 
  form,
  onFormSubmit,
  ...
  const [searchParams, setSearchParams] = useSearchParams()
  const [posCategory] = useAtom(posCategoryAtom)
  const [formState] = useAtom(posFormStateAtom)
  const selectedStep = searchParams.get("tab") || "overview"
  ...
  const currentStepIndex = steps.findIndex((step) => step.value === selectedStep)
  const prevStep = currentStepIndex > 0 ? steps[currentStepIndex - 1].value : null
  const nextStep = currentStepIndex < steps.length - 1 ? steps[currentStepIndex + 1].value : null
  ...
  const validateCurrentStep = (): boolean => {
    if (selectedStep === "properties" && form) {
      ...
      if (!values.allowTypes || values.allowTypes.length === 0) {
        setValidationError("Please select at least one type before proceeding.")
        return false
      }
    }
    return true
  ...
  const handleNextStep = async (): Promise<void> => {
    ...
    // Handle form submission for properties step
