...
const POSSlotsManager = ({ posId }: { posId: string }) => { // Add posId prop
  ...
  }, [updatePosSlots, posId, transformNodesToSlots, toast])
  ...
  useEffect(() => {
    const handleNodeResize = (event: Event) => {
      ...
      if (detail && detail.id) {
        ...
        if (selectedNodeRef.current && selectedNodeRef.current.id === detail.id) {
  ...
  }, [setNodes, setSidebarView, setSelectedNode, setSlotDetail])
  ...
  const onNodeClick: NodeMouseHandler = useCallback(
    (event, node) => {
      ...
      setSlotDetail({
        ...
        top: nodeData.data.positionY.toString(),
        left: nodeData.data.positionX.toString(),
  ...
  const handleSaveSlotDetail = useCallback(async () => {
    if (selectedNodeRef.current) {
      ...
      try {
        const slotsData = transformNodesToSlots(nodesRef.current.map((node) => {
          if (node.id === selectedNodeRef.current?.id) {
            return {
              ...node,
              position: { x, y },
  ...
  }, [slotDetail, setNodes, setSidebarView, setSelectedNode, toast, setActiveTab, updatePosSlots, posId, transformNodesToSlots])
  ...
  const handleDeleteSlot = useCallback(async (id: string) => {
    ...
    try {
      const remainingNodes = nodesRef.current.filter((node) => node.id !== id)
      const slotsData = transformNodesToSlots(remainingNodes)
  ...
  }, [setNodes, setEdges, setSidebarView, setSelectedNode, toast, setActiveTab, updatePosSlots, posId, transformNodesToSlots])

  const handleDuplicateSlot = useCallback((id: string) => {
