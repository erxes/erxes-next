...
export const CurrencyForm = ({
  ...
  const { spotRate, loading: exchangeRateLoading } = useGetExchangeRate({
    ...
    skip: !detail?.account?.currency || detail?.account?.currency === mainCurrency,
  });
  ...
  useEffect(() => {
    if (spotRate && amount) {
      form.setValue(`trDocs.${journalIndex}.details.0.currencyAmount`, amount / spotRate);
  ...
  useEffect(() => {
    if (!diffAmount) {
      form.setValue(`trDocs.${journalIndex}.follows`, trDoc.follows?.filter(f => f.type !== 'currencyDiff'));
      setFollowTrDocs((followTrDocs || []).filter(ftr => !(ftr.originId === trDoc._id && ftr.followType === 'currencyDiff')));
      return;
    }
  ...
  if (!detail?.account?.currency || detail?.account?.currency === mainCurrency) {
    return null;
  }
    ...
    <>
      ...
      <Form.Field
        ...
        render={({ field }) => (
          <Form.Item>
            ...
            <CurrencyField.ValueInput
              value={field.value ?? 0}
              onChange={(value) => handleCurrencyAmount(value || 0, field.onChange)}
            />
            <Form.Message />
          </Form.Item>
        )}
      ...
      {diffAmount !== 0 && (
        <Form.Field
          ...
          render={({ field }) => (
            <Form.Item>
              <Form.Label>{`${diffAmount > 0 ? 'Loss' : 'Gain'} account`}</Form.Label>
              <Form.Control>
                <SelectAccount
                  value={field.value || ''}
