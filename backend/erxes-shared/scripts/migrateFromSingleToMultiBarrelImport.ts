 ...
 * @param importSpecifier The module to import from (e.g., 'erxes-shared/utils')
...
function insertImportAtTop({
  ...
  let newImport = `import { ${namedImports.join(
    ', ',
...
const migrateImports = (mappedResolutions: MappedResolution[]) => {
  for (const { file, newImports } of mappedResolutions) {
    for (const { barrel, modules } of Object.values(newImports)) {
      ...
      removeSpecificImports({
        filePath: file,
        moduleSpecifier: 'erxes-shared',
      });
      insertImportAtTop({
        filePath: file,
        importSpecifier: `erxes-shared/${barrel}`,
...
const main = () => {
  const backendrcPath = 'backend/erxes-shared/src';
  const exportsPerModule = retrievePackageExportsPerModule(backendrcPath);

  const backendToMigrate = [
    'erxes-front',
    'erxes-ui',
    'erxes-server',
    'erxes-emails',
    'erxes-zapier',
    'erxes-chrome-extension',
  ];
  for (const currPackage of backendToMigrate) {
