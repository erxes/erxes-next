...
import { productUomSchema } from '../../definitions/products';

export interface IProductUomModel extends Model<IProductUomDocument> {
  getUom(selector: any): Promise<IProductUomDocument>;
  createUom(doc: IProductUom): Promise<IProductUomDocument>;
  updateUom(_id: string, doc: IProductUom): Promise<IProductUomDocument>;
  removeUoms(_ids: string[]): Promise<{ n: number; ok: number }>;
  checkUOM(doc: { uom?: string; subUoms?: any[] });
}

export const loadProductUomClass = (models: IModels, subdomain: string) => {
  class ProductUom {
    /**
     *
     * Get Uom Cagegory
     ...
     * Create a uom
     */
    public static async createUom(doc: IProductUom) {
     ...
     * Update Product
     */
    public static async updateUom(_id: string, doc: IProductUom) {
  ...
  productUomSchema.loadClass(ProductUom);

  return productUomSchema;
};
