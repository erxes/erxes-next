...
export const uploadToRag = async (file: File, userId?: string): Promise<RagUploadResponse> => {
  const formData = new FormData();
  formData.append('file', file);
  
  if (userId) {
    formData.append('org_id', userId);
  }
...
export const askRag = async (question: string, userId: string, topK: number = 3): Promise<RagAskResponse> => {
  try {
