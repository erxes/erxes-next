import { IUiOptions, IColorDefinition } from '@/types';
...
export class TailwindThemeManager {
   ...
   */
  applyUiOptions(uiOptions: IUiOptions): void {
    if (!uiOptions) return;

    // Apply primary colors
    if (uiOptions.primary?.DEFAULT) {
      this.applyPrimaryColors(uiOptions.primary);
    }
   ...
   */
  private applyPrimaryColors(primary: IColorDefinition): void {
    if (!primary.DEFAULT) return;

    try {
      const primaryHsl = ColorUtils.hexToHsl(primary.DEFAULT);
      ...
      const primaryForeground = primary.foreground || ColorUtils.getContrastColor(primary.DEFAULT);
   ...
   */
  private setCommonColorVariables(uiOptions: IUiOptions): void {
   ...
   */
  generateClasses(uiOptions: IUiOptions): TailwindClasses {
...
export const applyUiOptionsToTailwind = (uiOptions: IUiOptions): void => {
...
export const generateTailwindClasses = (uiOptions: IUiOptions): TailwindClasses => {
