...
export const stageQueries = {
  ...
  async tasksStages(
    ...
    if (!isAll) {
      filter.status = { $ne: TASK_STATUSES.ARCHIVED };
  ...
  async tasksArchivedStages(
    ...
    const filter: any = { pipelineId, status: TASK_STATUSES.ARCHIVED };

    if (search) {
      Object.assign(filter, regexSearchText(search, 'name'));
    }

    return await models.Stages.find(filter).sort({ createdAt: -1 }).lean();
  },
